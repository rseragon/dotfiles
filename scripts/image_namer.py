...
def main():
    ...
    files = [(folder_path+os.sep+file) for file in os.listdir(folder_path) if file.endswith(".jpg") or file.endswith(".png") or file.endswith(".jpeg") or file.endswith(".jfif")]
    ...
    for file in files:
        ...
        if(len(result) > 0):
            print(f"[{file}] Author: {result[index].author} title: {result[index].title} similarity: {result[index].similarity} url: {result[index].urls}")
            log.write(f"[{file}] Author: {result[index].author} title: {result[index].title} similarity: {result[index].similarity} url: {result[index].urls}\n")
            count += change_filename(folder_path, file, str(result[index].author), str(result[index].title))
        else:
...
# find a new name, if it already exists
    ...
    if(os.path.exists(new_name) == True):
        new_name = path + os.sep + str(author).title() + "_" + str(title).title() + str(random.randint(1, 1000)) + ext 
